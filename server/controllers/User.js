...
class User {
    async signup(req, res, next) {
        ...
        try {
            if (!email || !password) {
                throw new Error('Порожній email або пароль')
            }
            if (role !== 'USER') {
                throw new Error('Можлива лише роль USER')
            }
    ...
    async login(req, res, next) {
        try {
            ...
            if (!compare) {
                throw new Error('Вказано невірний пароль')
            }
            const token = makeJwt(user.id, user.email, user.role)
            return res.json({token})
        } catch(e) {
    ...
    async create(req, res, next) {
        ...
        try {
            ...
            if ( ! ['USER', 'ADMIN'].includes(role)) {
                throw new Error('Недопустиме значення ролі')
            }
            const hash = await bcrypt.hash(password, 5)
            const user = await UserModel.create({email, password: hash, role})
