...
const UpdateProperties = (props) => {
    ...
    const remove = (unique) => {
        // нову хар-ку треба просто видалити з массиву properties, а стару — залишити, але
        // змінити remove на true, щоб потім виконати http-запит на сервер для видалення
        const item = properties.find(elem => elem.unique === unique)
        if (item.id) { // стара хар-ка
