...
const Shop = observer(() => {
    ...
    // При кажному натисканні на категорію, видавництво або номер сторінки — додається елемент до іторії
    // браузера, посилання в исторії мають вигляд /?page=1, /?page=2, /?page=3. При натисканні кнопки 
    // «Назад» браузера — ми відслідковуєм зміни GET-параметров та змінюєм стан сховища.
    useEffect(() => {
        const {category, brand, page} = getSearchParams(searchParams)
    ...
    // при натисканні на категорію, видавництво, номер сторінки або при натисканні кнопки  «Назад» 
    // браузера — отримуємо з сервера список товарів, тому що це вже інший список
    useEffect(() => {
